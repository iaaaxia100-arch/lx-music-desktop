<template>
  <div :class="$style.list">
    ...
    <div v-if="selectedList.length" :class="$style.batchBar">
      <span :class="$style.batchInfo">{{ $t('lists__batch_selected', { n: selectedList.length }) }}</span>
      <base-btn :class="$style.batchBtn" @click="handleBatchRemove">{{ $t('lists__batch_remove') }}</base-btn>
      <base-btn :class="$style.batchBtn" @click="removeAllSelect">{{ $t('lists__batch_cancel') }}</base-btn>
    </div>
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const handleBatchRemove = () => {
      void handleRemoveMusic(-1, false)
    }
